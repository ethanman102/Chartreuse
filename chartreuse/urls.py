...
from .view import home_page_view, profile_utils, signup_view, login_view, landing_page_view, profile_view, follow_list_view, post_view, settings_view, post_utils, comment_utils, follow_utils,discover_view, inbox, user_list_view
app_name = "chartreuse"
urlpatterns = [
    ...
    # Github URLs
    re_path(r"github/(?P<user_id>.+\w)/events/", github.get_events, name="get_events"),
    re_path(r"github/(?P<user_id>.+\w)/starred/", github.get_starred, name="get_starred"),
    re_path(r"github/(?P<user_id>.+\w)/subscriptions/", github.get_subscriptions, name="get_subscriptions"),
    ...
    re_path(r"authors/(?P<url_id>https?.+)/post/(?P<post_id>https?.+\w)/liked-users/$",user_list_view.UserListDetailView.as_view(),name="users_who_liked"),
