...
from chartreuse.models import User,Follow,Post
...
class UserListDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if relationship not in ['likes','friends','following','followers']:
            raise Http404("Page not found")
        elif relationship == "followers":
            users = self.get_followers(page_user)
        elif relationship == "following":
            users = self.get_following(page_user)
        elif relationship == 'friends':
            users = self.get_friends(page_user)
        elif relationship == 'likes':
            users = self.get_liking_users()
        ...
        for user in users:
            user.url_id = quote(user.url_id,safe='')
    ...
    def get_object(self):
        user_id = unquote(self.kwargs.get('user_id',None))
        if user_id == None:
            post_id = unquote(self.kwargs.get('post_id',None))
            post = Post.objects.get(url=post_id)
            user = User.objects.get(user=post.user)
        return user
